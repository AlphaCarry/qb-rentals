    ...
    if license == "driver" then
        QBCore.Functions.TriggerCallback("qb-rentals:server:getDriverLicenseStatus", function(hasLicense)
            if hasLicense  then
                ...
                QBCore.Functions.Notify(Lang:t("error.no_driver_license"), "info", 4500)
            end
        end)
...
RegisterNetEvent('qb-rental:client:spawncar', function(data)
    ...
    if menu == "vehicle" then
        if IsAnyVehicleNearPoint(Config.Locations.vehicle.spawnpoint.x, Config.Locations.vehicle.spawnpoint.y, Config.Locations.vehicle.spawnpoint.z, 2.0) then
            QBCore.Functions.Notify(label, "info", 4500)
            return
        end
 --[[   elseif menu == "aircraft" then
        ...
        if money then
            if menu == "vehicle" then
                QBCore.Functions.SpawnVehicle(model, function(vehicle)
                    ...
                    TriggerServerEvent('vehiclekeys:server:givekey', GetVehicleNumberPlateText(vehicle), vehicle)
                    SetVehicleEngineOn(vehicle, true, true)
                    SetVehicleDirtLevel(vehicle, 0.0)
                    exports[Config.FuelExport]:SetFuel(vehicle, 100)
                    SpawnVehicle = true
					QBCore.Functions.Notify("Aracınız 20 dakika sonra silinecektir", "info")
				--	setTimerStarte = true
					Citizen.SetTimeout(60000*10, function()
					QBCore.Functions.Notify("Aracınız 10 dakika sonra silinecektir", "info")
					end)
					Citizen.SetTimeout(60000*20, function()
          ...
          --[[  elseif menu == "aircraft" then
            ...
            QBCore.Functions.Notify(Lang:t("error.not_enough_money"), "info")
        end
    end, money)
end)
...
RegisterNetEvent('qb-rental:client:return', function()
    if SpawnVehicle then
        ...
        QBCore.Functions.Notify(Lang:t("error.no_vehicle"), "info")
    end
    SpawnVehicle = false
end)
